...
export function ChatScreen() {
  const { apiClient, isAuthLoaded, sessionId } = useApiClient();
  ...
  const loadResources = useEffectEvent(async () => {
    ...
    try {
      const [repos, llmProviders, agents] = await Promise.all([
        apiClient.listRepos(),
        apiClient.listLLMProviders(),
        apiClient.listRegistryEntries()
      ]);
  ...
  const loadHistory = useEffectEvent(async (activeSessionId: string) => {
    ...
    try {
      const messages = await apiClient.readChatHistory(activeSessionId);
          ...
          <form className="chat-input-form" onSubmit={(event) => void handleSubmit(event)}>
            ...
            <div className="chat-input-row">
              <textarea
                ...
                rows={4}
                disabled={!isAuthLoaded || !sessionId || snapshot.isSending}
              />
              <button
                type="submit"
                className="chat-send-btn"
                disabled={
                  !isAuthLoaded || !sessionId || snapshot.isSending || inputValue.trim().length === 0
                }
              >
